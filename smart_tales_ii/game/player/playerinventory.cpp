...
bool Player::Inventory::CanAfford(const unsigned int amount) const { return amount <= currency; }

void Player::Inventory::AddSensorUpgrade(const Upgrade::Sensor s) { sensorUpgrades |= static_cast<uint8_t>(s); }

void Player::Inventory::RemoveSensorUpgrade(const Upgrade::Sensor s) { sensorUpgrades &= ~(static_cast<uint8_t>(s)); }
...
unsigned int Player::Inventory::GetCurrency() const { return currency; }

void Player::Inventory::AddCurrency(const unsigned int amount) { currency += amount; }
