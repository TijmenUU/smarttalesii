...
{
    Animation::Animation()
     : frameStart(), frameSize(), frameMargin(0U), frameCount(0U), frameTime(0U), reverse(false), loop(false)
    { }

#pragma region Animation Loading
    std::array<std::string, 8> cAnimationHeaders = {"spritesheet-source", "animation"};
...
#pragma endregion
                ...
                case AnimationHeader::SpritesheetSource:
                {
                    ...
                    if(!texture.loadFromFile(sourcefile))
                    {
                        throw std::runtime_error(
                            "Failed to load texture <" + sourcefile + "> specified in animation file <" + animationFile
                            + ">.");
                    }
                }
                break;
    ...
    const sf::Texture & Sheet::GetTexture() const { return texture; }
}
