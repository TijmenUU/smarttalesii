...
const std::array<std::string, 6> cDifficultyProperties
    = {"start-velocity", "velocity-control", "max-velocity", "hint-border", "velocity-increment-per-upgrade"};
...
float Difficulty::GetHintBorderXCoord() const { return hintBorderX; }

void Difficulty::LoadFromFile(const std::string & definitionFile)
{
    std::vector<std::string> lines = Util::LoadTextFile(definitionFile, true, true);
