...
bool RunningMode::SuppressDraw() const { return true; }

bool RunningMode::SuppressUpdate() const { return true; }
            ...
            if(!obstacle.IsUnharmful())
            {
                ...
                if(obstaclePosition.x <= gameDifficulty.GetHintBorderXCoord())
                {
                    ...
                    obstacleHintText.setPosition(
                        Util::GetCenterOffset(obstacleHintText.getLocalBounds(), hintPosition));
...
void RunningMode::Update(const sf::Time & elapsed, const Inputhandler & input)
{
    ...
    scrollVelocity = gameDifficulty.GetScrollVelocity(
        playerInventory.GetSensorUpgradeCount(),
