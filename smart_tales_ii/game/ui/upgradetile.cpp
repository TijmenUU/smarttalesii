...
sf::FloatRect UpgradeTile::GetGlobalBounds() const { return background.getGlobalBounds(); }

void UpgradeTile::Refresh(const Player::Inventory & inventory) { purchaseButton.Refresh(inventory); }
...
bool UpgradeTile::HandleInput(const Inputhandler & input, Player::Inventory & inventory)
{
    if(purchaseButton.HandleInput(input))
    {
        ...
        std::printf(
            "Bought upgrade for %u, making your balance %u.\n",
            purchaseButton.GetPrice(),
...
void UpgradeTile::Update(const sf::Time & elapsed, const float horizontalDisplacement)
{
