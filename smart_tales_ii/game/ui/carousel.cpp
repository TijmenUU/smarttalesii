...
void SaleTileCarousel::Update(const sf::Time & elapsed, const Inputhandler & input, Player::Inventory & inventory)
{
    ...
    else if(input.PointingDeviceReleasedEvent())
    {
        ...
        const float distanceTravelledTotal
            = VectorMathF::Distance(mouseStartPosition, input.PointingDeviceWorldPosition());
...
SaleTileCarousel::SaleTileCarousel() : leftTilePosition(cTileSpacing, cTileYPos) { }
