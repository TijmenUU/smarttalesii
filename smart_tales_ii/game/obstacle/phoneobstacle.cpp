...
namespace Obstacle
{
    ...
    void Phone::draw(sf::RenderTarget & target, sf::RenderStates states) const { target.draw(obstacleSprite, states); }
    ...
    sf::Vector2f Phone::GetHintPosition() const { return cLocalHintPosition + obstacleSprite.getPosition(); }
    ...
    Base * Phone::Clone() const { return new Phone(*this); }

    Phone::Phone(const bool playerHasSensor)
     : GestureBase(4, 50.f, Type::Phone, ResourceCache::GetInstance().GetSpriteSheet("phone"), playerHasSensor)
    { }
}
