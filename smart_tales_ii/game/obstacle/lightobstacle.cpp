...
namespace Obstacle
{
    ...
    Base * Light::Clone() const { return new Light(*this); }

    Light::Light(const bool playerHasSensor)
     : GestureSensorBase(
           ResourceCache::GetInstance().GetSpriteSheet("light"),
           ResourceCache::GetInstance().GetSpriteSheet("passive_ir"),
           2U,
           50.f,
           Type::Light,
           playerHasSensor),
       sensorBeam(ResourceCache::GetInstance().GetTexture("passive_ir_beam")), showBeam(playerHasSensor),
