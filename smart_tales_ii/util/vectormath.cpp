...
{
    float ToDegrees(const float radians) { return radians * cDegreesPerRad; }

    float ToRadians(const float degrees) { return degrees / cDegreesPerRad; }

    float ToRadians(const sf::Vector2f & vec) { return std::atan2(vec.y, vec.x); }

    sf::Vector2f RadiansToVector(const float radians) { return sf::Vector2f(std::cos(radians), std::sin(radians)); }

    sf::Vector2f DegreesToVector(const float degrees) { return RadiansToVector(ToRadians(degrees)); }
    ...
    sf::Vector2f RotateVector(const sf::Vector2f & vec, const float radians)
    {
        ...
        return sf::Vector2f(vec.x * cosRad - vec.y * sinRad, vec.x * sinRad + vec.y * cosRad);
    }

    float Length(const sf::Vector2f & vec) { return std::sqrt(vec.x * vec.x + vec.y * vec.y); }
    ...
    sf::Vector2f Clamp(const sf::Vector2f & vec, const float min, const float max)
    {
    ...
    float Dot(const sf::Vector2f & A, const sf::Vector2f & B) { return A.x * B.x + A.y * B.y; }
    ...
    bool ClockwiseCompare(const sf::Vector2f & center, const sf::Vector2f & a, const sf::Vector2f & b)
    {
...
{
    std::int32_t Clamp(const std::int32_t val, const std::int32_t min, const std::int32_t max)
    {
        if(val > max)
        {
            return max;
    ...
    sf::Vector2i Clamp(const sf::Vector2i & vec, const std::int32_t min, const std::int32_t max)
    {
        return sf::Vector2i(Clamp(vec.x, min, max), Clamp(vec.y, min, max));
    }

    std::int32_t ManhattanDistance(const sf::Vector2i & A, const sf::Vector2i & B)
    {
